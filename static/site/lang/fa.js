window.i18n = {
    "title": "محتوا را انتخاب کنید",
    "powered_by": "قدرت‌گرفته از Internet-in-a-Box",
    "books": "یک کتاب بخوانید",
    "code": "Code on the Go",
    "kiwix": "کاوش در Wikipedia",
    "kolibri": "شرکت در دوره‌ها",
    "maps": "پیمایش نقشه‌ها",
    "matomo": "تحلیل استفاده",
    "k2go_docs": "راهنما و راهنماها",
    "ready": "آماده",
    "connecting": "در حال اتصال",
    "loading": "در حال بارگذاری…",
    "opening": "در حال باز کردن",
    "theme_label": "پوسته",
    "theme_system": "سیستم",
    "theme_light": "روشن",
    "theme_dark": "تیره",
    "backend_error": "سرور K2Go پاسخ نمی‌دهد. مطمئن شوید K2Go روی دستگاه میزبان در حال اجراست؛ صفحه خودش دوباره متصل می‌شود. اگر بازنگشت، برای دریافت آدرس فعلی، کد را دوباره اسکن کنید."
};
