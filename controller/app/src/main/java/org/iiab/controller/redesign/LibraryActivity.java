...
public class LibraryActivity extends AppCompatActivity implements ServerController.Host {
    ...
    private boolean moduleBusy = false;   // ADFA-4842: a module (runrole) queue is running — block the UI
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // ADFA-4842: while a module (runrole) queue runs, the server is stopped and the rootfs is being
        // modified — block the WHOLE library UI (every tab + nav) behind the full-screen boot gate, show
        // which module is installing, and bring the server back when the queue finishes. The batch clears
        // InstallGuard before posting DONE, so canStartServer() passes here.
    ...
    // ADFA-4842: full-screen "system updating" block while a module (runrole) queue runs. The boot gate
    // is match_parent + clickable and drawn over the nav content, so it blocks EVERY tab (Library,
    // Connect, Clone, Settings) and the nav bar — not just the Library cards.
