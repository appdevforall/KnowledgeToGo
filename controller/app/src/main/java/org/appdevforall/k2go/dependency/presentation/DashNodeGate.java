 ...
 * Description : K2GO-430. Gate a module install batch on the box dash-node version. Two mechanisms:
 ...
 *               SOFT suggest : for any install, when a dashboard update is available, recommend updating
 *               first (Update / Install anyway). Reuses the existing version read (DashboardVersion, from
 *               the rootfs, no network/proot) and the cached update-check (UpdateStatusCache), so the gate
 *               is synchronous.
 * ============================================================================
 */
...
public final class DashNodeGate {
    ...
    private static void showBlocked(@NonNull Activity act, @NonNull String moduleKey) {
        new BrandDialog(act)
                .setTitle(R.string.k2go_dep_required_title)
                .setMessage(act.getString(R.string.k2go_dep_required_msg_fmt, DashNodeRequirement.minLabel(moduleKey)))
                .setPositive(R.string.k2go_dep_update_dashboard, BrandDialog.Role.PRIMARY, () -> openDashboard(act))
                .setNegative(android.R.string.cancel, null)
                .show();
    }

    private static void showSuggest(@NonNull Activity act, @NonNull Runnable onProceed) {
        new BrandDialog(act)
    ...
    /** Open the dashboard detail (the Rebuild/Update card) so the user can update the dash-node. Reuses
     *  the canonical navigation intent instead of rebuilding it here. */
    private static void openDashboard(@NonNull Activity act) {
        act.startActivity(OpReturnNavigator.dashboardRebuild(act));
    }
}
