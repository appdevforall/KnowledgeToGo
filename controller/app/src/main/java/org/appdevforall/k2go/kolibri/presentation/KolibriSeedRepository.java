...
public final class KolibriSeedRepository {
    ...
    public boolean isActiveNow() {
        return hasSession() && !isComplete()
                && org.appdevforall.k2go.env.Freshness.fresh(
                        lastProgressAtMs, android.os.SystemClock.elapsedRealtime(),
                        org.appdevforall.k2go.env.Freshness.STALE_MS);
    }
    ...
    public synchronized void itemProgress(int index, int percent, long speedBytesPerSec) {
        ...
        mutate(from -> {
            ...
            long derived = org.appdevforall.k2go.system.domain.TransferRate.perSecond(
