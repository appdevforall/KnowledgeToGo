    ...
    public void addNewSession() {
        ...
        try {
            ...
            try {
                ...
                org.appdevforall.k2go.network.domain.DnsConfig termDns;
                try {
                    termDns = new org.appdevforall.k2go.network.data.PrefsDnsConfigRepository(activity).loadEffective();
                } catch (Exception dnsEx) {
                    termDns = org.appdevforall.k2go.network.domain.DnsConfig.defaults();
                }
                if (termDns == null || termDns.isEmpty()) termDns = org.appdevforall.k2go.network.domain.DnsConfig.defaults();
                ...
                if (new org.appdevforall.k2go.proot.data.PrefsSeccompModeRepository(activity).load()
                        == org.appdevforall.k2go.proot.domain.SeccompMode.DISABLED) {
                    cliStr.append("export ")
                            .append(org.appdevforall.k2go.proot.data.ProotEnvironment.NO_SECCOMP)
                            .append("=1\n");
                }
                cliStr.append("\n");
...
//                cliStr.append("    fi\n\n");
                ...
                cliStr.append("    am broadcast -a org.iiab.ACTION_BACKUP_ROOTFS -p org.appdevforall.k2go >/dev/null 2>&1\n");
                cliStr.append("elif [ \"$ACTION\" = \"restore\" ]; then\n");
                cliStr.append("    echo -e '\\033[33m[iiab]\\033[0m Triggering restore in UI...'\n");
                cliStr.append("    am broadcast -a org.iiab.ACTION_RESTORE_ROOTFS -p org.appdevforall.k2go >/dev/null 2>&1\n");
                cliStr.append("else\n");
