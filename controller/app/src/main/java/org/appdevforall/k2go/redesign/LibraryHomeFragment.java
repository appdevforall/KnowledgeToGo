...
import org.appdevforall.k2go.system.data.PlatformEvidence;
...
public class LibraryHomeFragment extends Fragment {
    ...
    private void openGetMore() {
        ...
        if (org.appdevforall.k2go.env.EnvironmentLock.ownerHeld(requireContext())) {
            if (getView() != null) {
                org.appdevforall.k2go.util.Snackbars.make(getView(), R.string.k2go_install_busy).show();
            }
            return;
        }
        ...
        org.appdevforall.k2go.system.domain.SystemVerdict.State verdict =
                org.appdevforall.k2go.system.data.SystemFactsReader.verdict(requireContext());
        ...
        if (verdict == org.appdevforall.k2go.system.domain.SystemVerdict.State.READY) {
    ...
    private void openSheet(Card c) {
        ...
        else if (verdict == org.appdevforall.k2go.system.domain.SystemVerdict.State.NO_SYSTEM
        ...
        } else if (!org.appdevforall.k2go.system.data.SystemFactsReader.serverAnswering()) {
     ...
     */
    private org.appdevforall.k2go.system.data.PendingContent.Snapshot passContent;
     ...
     */
    private org.appdevforall.k2go.system.domain.SystemVerdict.State passVerdict =
            org.appdevforall.k2go.system.domain.SystemVerdict.State.READY;

    private void refreshStatuses() {
        boolean installed = org.appdevforall.k2go.SystemStateEvaluator.isSystemInstalled(requireContext());
        systemInstalled = installed;
        boolean alive = ServerStateRepository.get().current().alive;
        passContent = org.appdevforall.k2go.system.data.PendingContent.read(requireContext());
        passVerdict = org.appdevforall.k2go.system.data.SystemFactsReader.verdict(requireContext());
        ...
        if (passVerdict == org.appdevforall.k2go.system.domain.SystemVerdict.State.NO_SYSTEM
                || passVerdict == org.appdevforall.k2go.system.domain.SystemVerdict.State.DAMAGED) {
            for (final Card c : cards) {
    ...
    private void updateHeaderFromCards() {
        ...
        boolean contentRunning = passContent != null
                ...
                : org.appdevforall.k2go.system.data.PendingContent.anyRunning(requireContext());
        if (contentRunning) {
    ...
    private void applyState(Card c, int st) {
        ...
        if (systemInstalled
                ...
                && !org.appdevforall.k2go.system.data.SystemFactsReader.serverAnswering()) {
    ...
    private boolean contentInFlight(Card c) {
        ...
        org.appdevforall.k2go.system.domain.ContentType type =
                org.appdevforall.k2go.system.domain.ContentType.byEndpoint(c.endpoint);
