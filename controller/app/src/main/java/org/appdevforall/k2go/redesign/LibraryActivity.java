...
public class LibraryActivity extends AppCompatActivity implements ServerController.Host {
    ...
    private org.appdevforall.k2go.update.presentation.UpdateController updateController;
    ...
    private org.appdevforall.k2go.util.EllipsisAnimator bootEllipsis;
    private org.appdevforall.k2go.util.EllipsisAnimator readingEllipsis;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!broughtHereToSetUp
                && !org.appdevforall.k2go.system.data.SystemFactsReader.hereOrOnTheWay(this)) {
        ...
        if (savedInstanceState == null && !getIntent().hasExtra(EXTRA_TAB)
                && (org.appdevforall.k2go.sync.presentation.SyncProgressRepository.get().isActive()
        ...
        bootEllipsis = new org.appdevforall.k2go.util.EllipsisAnimator(installStatus, true);
        readingEllipsis = new org.appdevforall.k2go.util.EllipsisAnimator(installDetail, true);
        ...
        final boolean systemInstalled = org.appdevforall.k2go.SystemStateEvaluator.isSystemInstalled(this);
        ...
        recovering = !installing
                ...
                && org.appdevforall.k2go.InstallGuard.isInterrupted(this)
                && !InstallProgressRepository.get().current().isRunning()
                ...
                && !org.appdevforall.k2go.env.EnvironmentLock.ownerHeld(this);
        android.util.Log.i("K2Go-Recover", "onCreate recovering=" + recovering
                + " interrupted=" + org.appdevforall.k2go.InstallGuard.isInterrupted(this)
                + " systemInstalled=" + systemInstalled
                + " instRunning=" + InstallProgressRepository.get().current().isRunning()
                + " modRunning=" + org.appdevforall.k2go.install.presentation.ModuleQueueRepository.get().isRunning());
        ...
        ServerStateRepository.get().state().observe(this, s -> {
            ...
            if (closing) {
                ...
                if (recovering) { recovering = false; org.appdevforall.k2go.InstallGuard.end(this); }
                onServerReady();
            }
        });
        ...
        InstallProgressRepository.get().state().observe(this, st -> {
            ...
            if (st.isRunning()) {
                ...
                if (st.phase == InstallState.Phase.FAILED
                        && org.appdevforall.k2go.InstallGuard.isLive(this)   // ADFA-5343 (Phase 5a): this process's own install just failed
                ...
                if (st.phase == InstallState.Phase.SUCCESS) {
                    ...
                    org.appdevforall.k2go.env.ServerLifecycleReconciler.get().setUserWantsOn(this, true);
        ...
        if (installing) {
            ...
            boolean nothingToBoot = !org.appdevforall.k2go.SystemStateEvaluator.rootfsPresent(this);
            if (!nothingToBoot) {
                ...
                org.appdevforall.k2go.env.ServerLifecycleReconciler r =
                        org.appdevforall.k2go.env.ServerLifecycleReconciler.get();
        ...
        } else if (org.appdevforall.k2go.env.EnvironmentLock.ownerHeld(this)) {
    ...
    private void showInstallProgress(InstallState st) {
        ...
        if (st.isHeld()) {
            ...
            if (st.percent < 0) {
                ...
                installDetail.setText(org.appdevforall.k2go.deploy.domain.ExtractProgress.fileLabel(st.message));
            }
        } else {
    ...
    protected void onStop() {
        ...
        if (userPresenceSent && InstallProgressRepository.get().current().isSoftFailed()) {
            ...
            sendToInstallService(
                    org.appdevforall.k2go.install.presentation.InstallService.ACTION_USER_ABSENT);
        }
    }
    ...
    public void onUserInteraction() {
        ...
        sendToInstallService(org.appdevforall.k2go.install.presentation.InstallService.ACTION_USER_PRESENT);
    }
    ...
    private void onDownloadToggle() {
        ...
        if (st.isHeld()) {
            sendToInstallService(org.appdevforall.k2go.install.presentation.InstallService.ACTION_RESUME);
        } else if (st.phase == InstallState.Phase.DOWNLOADING) {
            sendToInstallService(org.appdevforall.k2go.install.presentation.InstallService.ACTION_PAUSE);
        }
    }
    ...
    private void sendToInstallService(String action) {
        try {
            startService(new Intent(this,
                    org.appdevforall.k2go.install.presentation.InstallService.class).setAction(action));
        } catch (Exception e) {
    ...
    private void onServerReady() {
        ...
        org.appdevforall.k2go.feedback.presentation.FeedbackFab.installOn(this, "library", 88);
    ...
    public org.appdevforall.k2go.update.presentation.UpdateController updateController() {
    ...
    private void evaluateRecovery() {
        ...
        recovering = false;
        boolean interrupted = org.appdevforall.k2go.InstallGuard.isInterrupted(this);
        boolean reachable = ServerStateRepository.get().current().alive;
        org.appdevforall.k2go.install.domain.InterruptedInstallDetector.Verdict v =
                org.appdevforall.k2go.install.domain.InterruptedInstallDetector.evaluate(interrupted, reachable);
        android.util.Log.i("K2Go-Recover", "verdict=" + v + " interrupted=" + interrupted + " reachable=" + reachable);
        if (v == org.appdevforall.k2go.install.domain.InterruptedInstallDetector.Verdict.DAMAGED_REINSTALL) {
            ...
            org.appdevforall.k2go.env.ServerLifecycleReconciler.get().setUserWantsOn(this, false);
            showDamagedDialog();
        } else {
            if (interrupted) org.appdevforall.k2go.InstallGuard.end(this);   // stale marker — system is usable
            onServerReady();
        }
    }
    ...
    private String installFailureReport() {
        java.util.List<String> all = org.appdevforall.k2go.LogRepository.get().snapshot();
    ...
    private boolean reduceMotion() {
        ...
        return org.appdevforall.k2go.util.Motion.reduced(this);
    }
    ...
    public boolean isServerStarting() {
        return org.appdevforall.k2go.env.ServerLifecycleReconciler.get().lastDesiredUp()
    ...
    public boolean canStartServer() {
        return !closing
                ...
                && !org.appdevforall.k2go.InstallGuard.isLive(this)   // ADFA-5343 (Phase 5a): don't start over a LIVE install; an interrupted one is recovery's to resolve
                ...
                && !org.appdevforall.k2go.install.presentation.ModuleQueueRepository.get().isRunning()
                && !org.appdevforall.k2go.env.EnvironmentLock.ownerHeld(this)
                && org.appdevforall.k2go.SystemStateEvaluator.isSystemInstalled(this);
    }
