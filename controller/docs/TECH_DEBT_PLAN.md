...
_Last updated: 2026-07-09. Tracks remediation work against the findings below. IDs map to the register in this file (F/D/S/M) and to `FORK_DELTA_ANALYSIS.md` (K)._
...
**M8 + M19 + M13 — Phase-4 build/flag hygiene: DONE / corrected** (PR `chore/phase4-quickwins`; ADFA-1028)
- **M8** (dead `jcenter()`): removed both declarations (buildscript + allprojects) in `controller/build.gradle`. `google()`/`mavenCentral()`/`jitpack` remain and nothing resolved from jcenter (sunset 2021).
- **M19** (`DEBUG_ENABLED` hardcoded `true`): `IIABWatchdog.DEBUG_ENABLED` is now `= BuildConfig.DEBUG`, so the heartbeat/blackbox debug logging is off in release builds.
- **M13 — WITHDRAWN (not real debt):** the "dead `ServiceReceiver` with null-action NPE" no longer applies. After the VPN removal (ADFA-4552) the receiver was repurposed as the documented seam for boot auto-start (ADFA-3340); it now guards a null action and is intentionally kept (unregistered until 3340 lands). Do NOT delete.
